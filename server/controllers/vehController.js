...
const addVeh = (req, res) => {
  ...
  newVehicle.save((error, _veh) => {
    if (error)
      return res
        .status(400)
        .json({ msg: `While saving new vehicle something went wrong`, error });
...
const getVehicleByVehicleId = (req, res) => {
  ...
  if (vehicleId) {
    Vehicle.findOne({ vehicleId: vehicleId }).exec((error, _vehicle) => {
      if (error)
...
const getAllVehicles = async (req, res) => {
  ...
  if (allVehicles) return res.status(200).json({ allVehicles });
    ...
    return res
      .status(400)
      .json({ msg: `Something wrong happened while fectching all vehicles` });
};
